...
class ChannelGate(nn.Module):
    ...
    def forward(self, x):
        ...
        channel_att_sum = channel_att_sum.reshape(channel_att_sum.shape[0], 2, 4)
        avg_weight = torch.mean(channel_att_sum, dim=2).unsqueeze(2)
        avg_weight = avg_weight.expand(channel_att_sum.shape[0], 2, 4).reshape(channel_att_sum.shape[0], 8)
